...
// Update the WebSocket proxy to handle RDCleanPath
(function() {
    const existingProxy = window.WebSocket;
    
    window.WebSocket = new Proxy(existingProxy, {
        construct(target, args) {
            const url = args[0];
            console.log('WebSocket proxy checking:', url);
            ...
            if (url && url.includes('rdcleanpath.proxy.local')) {
                ...
                return new RDCleanPathProxyWebSocket(url);
            }
            
            // Use existing proxy handler
            return new target(...args);
        }
    });
})();

console.log('RDCleanPath Proxy WebSocket handler installed');