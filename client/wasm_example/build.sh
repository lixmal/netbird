...
# Build the grpc-proxy
echo "Building grpc-proxy..."
go build -o grpc-proxy-bin ./grpc-proxy
if [ ! -f "grpc-proxy-bin" ]; then
    echo "Warning: grpc-proxy build failed"
fi

# Build the test server
echo "Building test server..."
(cd test_srv && go build -o test_server test_server.go)
if [ ! -f "test_srv/test_server" ]; then
    echo "Warning: test server build failed"
fi
